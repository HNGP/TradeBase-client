...
const Card = props => {
    ...
    const fetchApi = (title) => {
        ...
        let API_CALL = `https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&interval=5min&apikey=${APIkey}&symbol=${title}`;
        ...
        fetch(API_CALL)
            ...
            .then(data => {
                for (var key in data['Time Series (5min)']) {
                    stockX.push(key);
                    stockY.push(data['Time Series (5min)'][key]['4. close']);
